...
describe('auth', () => {
  beforeEach(async () => {
    ...
    limitedUser = await User.query()
      .upsertGraph(await fakeUser(profile2.id, [roleTokenRefresh]), userGraphUpdateOptions)
  ...
  describe('[POST] /auth/login', async () => {
    const data = [
      {name: 'valid by user', get usernameOrEmail () { return user.username }},
      {name: 'valid by email', get usernameOrEmail () { return profile.email }}
    ]
  ...
  describe('[POST] /auth/login', async () => {
    const data = [
      {name: 'invalid username', get usernameOrEmail () { return 'junk' }, get password () { return user.password }},
      {name: 'invalid password', get usernameOrEmail () { return profile.email }, get password () { return 'junk' }}
    ]
