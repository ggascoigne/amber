...
import { ConfigurationValue, ConventionYear, ConventionsDatesFull, ContactEmail } from '@amber/amber/components'
import { SlotDescriptionList } from '@amber/amber/components/SlotDescriptionList'
...
<ConventionsDatesFull pre='from' intra='through' post='.' /> There will be virtual "tours" of the Discord space and
introductions to use of Discord led by volunteers in the weeks preceding the convention. Games may take place using
Discord with or without video or through other technology as organized separately by the GMs.
...
[comment]: <> (We are charging **$<ConfigurationValue field='virtualCost' skipSpace='before'/>** to support ACNW's annual expenses.)

Virtual AmberCon NW is **FREE** this year.

The convention will be divided into <ConfigurationValue field='numberOfSlots'/> 4-hour event slots:
