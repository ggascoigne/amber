...
import { ConfigurationValue, ConventionOrdinal, ConventionYear, ConfigDate, RedStar } from '@amber/amber/components'
import { RoomsTable } from '@amber/amber/components/Rooms'
import { BathroomType } from '@amber/amber/utils'
...
the convention will contribute <ConfigurationValue field='gameRoomCredit'/> towards the cost.
...
McMenamins adds a <ConfigurationValue field='moreThanDoubleOccupancySurcharge'/> charge for each person over 2 in
room. Price shown does not include <ConfigurationValue field='oregonHotelTax'/> hotel tax.
...
a <ConfigurationValue field='gameRoomCredit' /> credit towards the room cost. Be advised that while we will do our best
to make sure that the rooms are used for games you are actually in, in exchange for the credit the game space will be
scheduled **at the discretion of the organizers**.
