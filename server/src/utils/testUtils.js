...
export function truncateTables () {
  return knex.transaction((txn) => {
    ...
    knex.raw('SELECT f_truncate_tables(?)', [config.get('database.username')])
      .transacting(txn)
      .then(txn.commit)
      .catch((e) => {
...
export async function prepTestDatabaseOnce () {
  if (!doneTruncation) {
    ...
    roleAdmin = await Role.query()
      .insert({
        authority: 'ROLE_ADMIN'
      })

    roleUser = await Role.query()
      .insert({
        authority: 'ROLE_USER'
      })

    roleTokenRefresh = await Role.query()
      .insert({
...
export function loadTestPlugins (server, pluginToTest) {
  return new Promise((resolve, reject) => {
    server.register([
      ...
      require('../plugins/response-update'),
      pluginToTest
    ], (error) => {
      if (error) {
        return reject(error)
