...
export async function getUsersHandler (req, reply) {
  try {
    const users = await User.query().eager('[profile, roles]')
...
export const roleNamesToRoles = async names => {
  ...
  return _.compact(names.map(n => roles.find(r => r.authority === n)))
}

const processPayload = async data => {
  const roles = await roleNamesToRoles(data.roles)
  return data.password ? { ...data, roles, password: await User.hashPassword(data.password) } : { ...data, roles }
}
