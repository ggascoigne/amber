...
export async function up(knex) {
  Promise.allSettled([
    ...
    knex.schema.createTable('member_hotel_room_assignment', (table) => {
      table.bigInteger('member_id').notNullable().references('membership.id').unsigned().index()
      table.bigInteger('hotel_room_id').notNullable().references('hotel_room_details.id').unsigned().index()
