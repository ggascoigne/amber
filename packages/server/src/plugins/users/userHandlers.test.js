...
describe('users', () => {
  ...
  test('[GET] /api/users/{id}', async () => {
    ...
    expect(payload.user.roles.map(r => r.authority)).toEqual(expect.arrayContaining([roleUser.authority]))
  })
  ...
  test('[GET] /api/users', async () => {
    ...
    expect(response.statusCode).toBe(200)
    const payload = JSON.parse(response.payload)
    expect(payload.users.map(u => u.username)).toEqual(expect.arrayContaining([user.username]))
  })
  ...
  test('[POST] /api/users - no role', async () => {
    const data = {
      username: faker.name.findName(),
      password: 'something1',
      profile_id: profile2.id
  ...
  describe('[POST /api/users - validation', () => {
    ...
    postPutData.forEach(run => {
      test(`[POST] /api/users - validation '${run.name}'`, async () => {
        const response = await server.inject({
          method: 'POST',
          url: `/api/users`,
          payload: run.payload
  ...
  test('hash', async () => {
    expect(await User.hashPassword('password')).toEqual(
