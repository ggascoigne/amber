...
const domain = (process.env.AUTH0_ISSUER_BASE_URL ?? process.env.AUTH0_DOMAIN ?? '').replace(/^https?:\/\//, '')
const appBaseUrl = toHttpsUrl(process.env.AUTH0_BASE_URL ?? process.env.VERCEL_URL ?? process.env.APP_BASE_URL)
